...
const BASE_URL = 'https://www.bydlet.cz';
const CATEGORIES = [
      ...
      for (const item of listItems) {
        try {
          ...
          ads.push(createAdObject({
            source: 'bydlet', url: item.adUrl, title: item.title, description: fullDesc || item.title,
          ...
          errors.push(`Bydlet detail error ${item.adUrl}: ${e.message}`);
        }
      }
    } catch(e) {
      if (e.message && e.message.includes('404')) continue;
      errors.push(`Bydlet cat ${cat.path}: ${e.message}`);
    }
  }
  console.log(`Bydlet.cz: nalezeno ${ads.length} inzerátů, ${errors.length} chyb`);
  return { ads, errors, source: 'bydlet' };
}
