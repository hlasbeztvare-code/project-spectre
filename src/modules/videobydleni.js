...
const BASE_URL = 'https://www.videobydleni.cz';
const CATEGORIES = [
      ...
      for (const item of listItems) {
        try {
          ...
          ads.push(createAdObject({
            source: 'videobydleni', url: item.adUrl, title: item.title, description: fullDesc || item.title,
          ...
          errors.push(`Videobydleni detail error ${item.adUrl}: ${e.message}`);
        }
      }
    } catch(e) {
      if (e.message && e.message.includes('404')) continue;
      errors.push(`Videobydleni cat ${cat.path}: ${e.message}`);
    }
  }
  console.log(`Videobydleni: nalezeno ${ads.length} inzerátů, ${errors.length} chyb`);
  return { ads, errors, source: 'videobydleni' };
}
