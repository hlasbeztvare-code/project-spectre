...
const BASE_URL = 'https://www.realcity.cz';
const CATEGORIES = [
      ...
      for (const item of listItems) {
        try {
          ...
          ads.push(createAdObject({
            source: 'realcity', url: item.adUrl, title: item.title, description: fullDesc || item.title,
          ...
          errors.push(`Realcity detail error ${item.adUrl}: ${e.message}`);
        }
      }
    } catch(e) {
      if (e.message && e.message.includes('404')) continue;
      errors.push(`Realcity cat ${cat.path}: ${e.message}`);
    }
  }
  console.log(`Realcity: nalezeno ${ads.length} inzerátů, ${errors.length} chyb`);
  return { ads, errors, source: 'realcity' };
}
